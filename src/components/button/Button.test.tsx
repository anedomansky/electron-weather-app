...
test('Fires the onClick event', () => {
    const { container, getByTestId } = render(<Button type="button" testId="test-btn" onClick={() => null} />);
    expect(container).toBeInTheDocument();
    fireEvent.click(getByTestId('test-btn'));
});

test('Fires the onKeyDown event', () => {
    const { container, getByTestId } = render(<Button type="button" testId="test-btn" onKeyDown={(e) => ((e.key === 'Enter' || e.keyCode === 13) ? 'correct' : null)} />);
    expect(container).toBeInTheDocument();
